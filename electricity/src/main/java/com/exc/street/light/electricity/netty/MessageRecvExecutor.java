...
//    private final static Logger logger = LoggerFactory.getLogger(MessageRecvExecutor.class);
    ...
    public MessageRecvExecutor(int localPort) {
...
//                    logger.debug("创建ThreadPoolExecutor线程池成功");
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        EventLoopGroup worker = new NioEventLoopGroup(16, threadRpcFactory, SelectorProvider.provider());
        ...
        try {
            ...
            ChannelFuture future = bootstrap.bind(this.localPort).sync();
            log.info("Netty EXC Server start success port:{}\n", this.localPort);
//                System.out.printf("[author xujiahao] Netty EXC Server start success ip:%s port:%d\n", host, port);
            /**
             * 这里会一直等待，直到socket被关闭
             */
